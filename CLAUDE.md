...
- `APPROACH`: drive at `GO_PCT`, steering by `STEER_K` per column the blob
  ...
  = `ARRIVED`; no blob for `LOST_TICKS` = back to `SCAN`. (A floor-line
  rule — arrive when the blob's lowest row touches row 0, the bottom of
  the frame, a size-independent distance cue — was tried and reverted as
  not enough better to earn its complexity; it's in git history.)
- `ARRIVED`: motors off. Blob shrinking below `FILL_PX - FILL_HYST_PX` =
  follow (`APPROACH`); gone for `LOST_TICKS` = `SCAN`; and after
  `ARRIVED_MAX_S` regardless = `SCAN`, the guard against sitting forever
  on something warm the frozen reference can't see past.
